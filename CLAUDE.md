...
## Особенности verifyStep

- `lcxiM` не возвращает имена файлов — только `mediaKey`, thumbnail URL, timestamp (`ci[2]`), `dedupKey` (`ci[3]`). Матчинг по имени возможен только через `batchQuotaInfo` (`qi[2]`).
- Для верификации нужно обойти всю библиотеку и архив через `enumerateAll` (не через ручной цикл с `allowEmpty` — он ломается на пустых ответах посередине). После получения всех `mediaKey` запускается `batchQuotaInfo` чанками, стоп — как только найдены все items.
- Если items в manifest имеют `isArchived: true` — нужно сканировать и архив (`enumerateAll` с `archive: true`), иначе фото не найдутся. После re-upload с Pixel архивные фото возвращаются в архив.
- `dedupKey` может измениться после re-upload если EXIF был изменён — матчинг по `dedupKey` ненадёжен, основной способ — по имени файла через `batchQuotaInfo`.
